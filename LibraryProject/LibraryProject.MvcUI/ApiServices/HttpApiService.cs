...
namespace LibraryProject.MvcUI.ApiServices
{
    public class HttpApiService : IHttpApiService
    {
        ...
        public async Task<bool> DeleteData(string requestUri, string token = null)
        {
            var requestMessage = new HttpRequestMessage()
            {
                ...
                RequestUri = new Uri($"http://localhost:5195/api{requestUri}"),
                Headers =
                {
        ...
        public Task<T> PutData<T>(string requestUri, string token = null)
        {
